#!/usr/bin/env python3
"""
Extract Diana's 8 vests/armors (ITEM_VEST 0~7) from the uploaded spritesheet.
Vests 0~3: 2x2 tiles (64x64 RGBA PNG)
Vests 4~7: 2x3 tiles (64x96 RGBA PNG)
"""
import os
import shutil
...
IMAGE_PATH = r'C:\Users\polyp\.gemini\antigravity\brain\72bb5645-bbbe-4c05-bdda-08bf1d5a7cdf\.user_uploaded\media_1789960062490.png'
...
def main():
    ...
    if len(all_vests) != 8:
        print(f"Error: Expected 8 vests, found {len(all_vests)} (top: {len(top)}, bot: {len(bot)})")
        return

    prev_im = Image.new('RGBA', (4 * 70 + 10, 70 + 104 + 16), (30, 30, 45, 255))

    for i, (xmin, ymin, xmax, ymax, pts) in enumerate(all_vests):
        ...
        out_name = os.path.join(RES_DIR, f'item_vest_diana{i}.png')
        canvas.save(out_name, 'PNG')
        if os.path.exists(DEBUG_RES_DIR):
            shutil.copy2(out_name, os.path.join(DEBUG_RES_DIR, f'item_vest_diana{i}.png'))
    ...
    prev_path = r'C:\Users\polyp\.gemini\antigravity\brain\72bb5645-bbbe-4c05-bdda-08bf1d5a7cdf\diana_vest_preview.png'
    prev_im.save(prev_path, 'PNG')
    print('All 8 Diana vests extracted and preview saved.')
