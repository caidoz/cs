...
void DrawObj(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i;
	...
	case PLAYERDRAW:
		...
		break;
	case NORMALDRAW:
		NormalDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case BULLETLASERDRAW:
		BulletLaserDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case BULLETGUIDEDDRAW:
		BulletGuidedDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case BULLETSATELITEDRAW:
		BulletSateliteDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case BULLETHEALDRAW:
		BulletHealDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case BULLETBOOMERANGDRAW:
		BulletBoomerangDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case BULLETITEMDRAW:
	...
	case CREWDRAW:
		...
		{
			DrawEffect(DEBUF_STUN0 + frame % 6, pObj->x, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y + 0 * _2X - ry - OBJIMGGAP), pObj->dirX, false, pObj->zoom, cvtDest, cvtLayer, buffering);
		}
		break;
	case ENEMYDRAW:
		...
		case ENEMY_MAMMOTH_BLACK:
			if (pObj->moveHandler == ENEMYMOVE) {
				ShieldBarDraw(pObj->shield, pObj->shieldMax, pObj->x - rx - (SIMPLEHPBARWIDTH / 2), STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y + pObj->cpy - 30 * _2X * pObj->zoom) - ry, pObj->zoom, COLOR_SKY, cvtDest, cvtLayer, buffering);
			}
			break;
		...
		case ENEMY_CASTLE_BOSS1_BLACK:
			...
			if (pObj->moveHandler == DEBRIONARMMOVE) {
				...
				case 1:
					SimpleHpBarDraw(pObj->hp, pObj->maxhp, pObj->x - rx - (48 * _2X), STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - 48 * _2X) - ry, pObj->zoom, ENEMYHPBARCOLOR, cvtDest, cvtLayer, buffering);
					break;
				case 2:
					SimpleHpBarDraw(pObj->hp, pObj->maxhp, pObj->x - rx + (28 * _2X), STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - 48 * _2X) - ry, pObj->zoom, ENEMYHPBARCOLOR, cvtDest, cvtLayer, buffering);
					break;
				case 3:
					SimpleHpBarDraw(pObj->hp, pObj->maxhp, pObj->x - rx - (48 * _2X), STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y + 32 * _2X) - ry, pObj->zoom, ENEMYHPBARCOLOR, cvtDest, cvtLayer, buffering);
					break;
				case 4:
					SimpleHpBarDraw(pObj->hp, pObj->maxhp, pObj->x - rx + (28 * _2X), STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y + 32 * _2X) - ry, pObj->zoom, ENEMYHPBARCOLOR, cvtDest, cvtLayer, buffering);
					break;
				}
				...
				SimpleHpBarDraw(pObj->hp, pObj->maxhp, pObj->x - rx - (float)(SIMPLEHPBARWIDTH + 2 * _2X) / 2, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y + 8 * _2X) - ry, pObj->zoom, ENEMYHPBARCOLOR, cvtDest, cvtLayer, buffering);
			}
			break;
		...
		case ENEMY_CASTLE_BOSS3_BLACK:
			if (pObj->moveHandler == FOLLOWMOMMOVE && pObj->jumpFrame > 0) {
				SimpleHpBarDraw(pObj->hp, pObj->maxhp, pObj->x - rx + 54 * _2X, pObj->y - ry + (pObj->motion == PO_C50_UPSKIN ? -130 * _2X : 0), pObj->zoom, ENEMYHPBARCOLOR, cvtDest, cvtLayer, buffering);
				...
				SimpleHpBarDraw(pObj->hp, pObj->maxhp, pObj->x - rx - 10 * _2X, pObj->y - ry - 20 * _2X, pObj->zoom, ENEMYHPBARCOLOR, cvtDest, cvtLayer, buffering);
		...
		break;
	case CLOUDDRAW:
		CloudDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case SPIDERDRAW:
		SpiderDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case SKELDRAW:
		SkelDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case MACHINEDRAW:
		MachineDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case LASERDRAW:
		LaserDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case GHOSTDRAW:
		GhostDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case VANISHDRAW:
		VanishDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case REGENDRAW:
		RegenDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case NEUTRALDRAW:
		NeutralDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	case BOXDRAW:
		BoxDraw(pObj, cvtDest, cvtLayer, buffering);
		break;
	}
	...
	if (obj < NEUTRAL) {
		...
		if (pObj->attr && pObj->moveHandler < BULLET3WAYMOVE) {
			...
			aType--;
			DrawEffect(attrEffect[aType * 20 + aFrame], pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);
			UnSetBlend();
		}
			...
			if (pObj->debuf[i] && dMotion > 0)
				DrawEffect(dMotion, xOffset + pObj->x - rx +
					(((pObj->type == ENEMY_CASTLE_BOSS3
						...
						&& pObj->moveHandler != BUGMOVE) ? (float)35 * _2X * pObj->zoom : 0 * _2X), STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y + (i == SLOW ? Abs(3 - (float)(pObj->debuf[SLOW] % 5) * _2X * pObj->zoom) - 4 * _2X * pObj->zoom : 0) - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);
	...
	//충돌영역 그리기
#ifdef GUIDELINE
	MemRectFrame(xOffset + pObj->x + pObj->cpx - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y + pObj->cpy - ry - OBJIMGGAP), pObj->cx, pObj->cy, 0xFFFF00, cvtDest, cvtLayer, buffering);
	MemRectFrame(xOffset + pObj->x + pObj->apx - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y + pObj->apy - ry - OBJIMGGAP), pObj->ax, pObj->ay, 0xFF0000, cvtDest, cvtLayer, buffering);
#endif
}

void DrawPlayer(OBJECT* pObj, int motion, int x, int y, int dirF, float zoom, float rotation, bool center, bool shadow, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i, fixedImg;
	int costume;
	int type;
	...
	if (shadow == true)
		ShadowImage(24 * _2X, 16 * _2X, 1 * _2X, 1 * _2X, x - (float)(12 * _2X) * zoom, y + (float)(8 * _2X) * zoom, SHADOW_IMG, zoom, cvtDest, cvtLayer, buffering);
...
//투명도 상대조절
		...
		if (imgFile == ROBIN_PART_IMG + ROBIN_IMG_WEAPON && motion >= PO_C0_A0 && motion <= PO_C0_A11 && concentrateMotion[pObj->concentrate]) {
			...
			DrawPlayer(pObj, (pObj->concentrate == 24 ? PO_C0_CONCENTRATE5 + robin.playtime % 3 : concentrateMotion[pObj->concentrate]) + ((motion >= PO_C0_A8) ? 8 : 0), x - DIR(dirF) * (*(cPtr + 1) * zoom * extra + imgOffsetX), imgOffsetY, dirF, zoom, false, false, false, cvtDest, cvtLayer, buffering);
			SetAlpha(32);
		}
...
void DrawPlayerIcon(int type, int x, int y, int zoom, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	clipX3 = clipX;
	clipY3 = clipY;
	clipX4 = clipX2;
	...
	DrawFrame(x, y, 24 * _2X * zoom, 20 * _2X * zoom, FRAME_SHOPBALLOON, cvtDest, cvtLayer, buffering);
	SetSectionClip(x + 2 * _2X, y - 2 * _2X, 24 * _2X * zoom - 4 * _2X, 20 * _2X * zoom - 4 * _2X, false);
	ao[type].head = true;
	DrawPlayer(&ao[type], 0, x + 2 * _2X + 14 * _2X * zoom, y - 32 * _2X * zoom, LEFT, zoom, false, false, false, cvtDest, cvtLayer, buffering);
	ao[type].head = false;
	UnSectionClip(false);
		...
		if (!tempAlpha) {
			...
			case ENEMY_CASTLE_BOSS4_BLACK:
				...
				if (fixedImg == IMG_C51_39) {
					DrawPlayer(&ao[NearPlayer(pObj)], ao[NearPlayer(pObj)].motion, ao[NearPlayer(pObj)].x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (ao[raidPlayer].y - OBJIMGGAP) - ry, ao[NearPlayer(pObj)].dirF, ao[NearPlayer(pObj)].zoom, false, false, false, cvtDest, cvtLayer, buffering);
				}
				break;
			}
		}
...
void DrawCommonShadow(int cmf, int x, int y, int dirF, float zoom, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	...
	if (IsBigCmf(cmf) == true)
		ShadowImage(40 * _2X, 16 * _2X, 26 * _2X, 1 * _2X, x - (float)20 * _2X * zoom, y + (float)10 * _2X * zoom, SHADOW_IMG, zoom, cvtDest, cvtLayer, buffering);
	else
		ShadowImage(24 * _2X, 16 * _2X, 1 * _2X, 1 * _2X, x - (float)12 * _2X * zoom, y + (float)10 * _2X * zoom, SHADOW_IMG, zoom, cvtDest, cvtLayer, buffering);
	SetAlpha(32);
}
...
{
	if (IsBigCmf(cmf) == true)
		ShadowImage(40 * _2X, 16 * _2X, 26 * _2X, 1 * _2X, x - (float)20 * _2X * zoom, y + (float)8 * _2X * zoom, SHADOW_IMG, zoom, cvtDest, cvtLayer, buffering);
	else
		ShadowImage(24 * _2X, 16 * _2X, 1 * _2X, 1 * _2X, x - (float)12 * _2X * zoom, y + (float)8 * _2X * zoom, SHADOW_IMG, zoom, cvtDest, cvtLayer, buffering);

	DrawCmfDetail(cmf, motion, x, y, dirF, zoom, false, false, cvtDest, cvtLayer, buffering);
}

void DrawEffect(int idx, int x, int y, int dirF, float rotation, float zoom, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
...
void DrawBgEffect(int idx, int x, int y, int dirF, float zoom, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	...
	do {
		...
		DrawImage(*(offPtr + 2), *(offPtr + 3), *offPtr, *(offPtr + 1), drawx, drawy, dirX, partsRotation, 0, pxl, tempAlpha, magnify, sprite[imgFile], cvtDest, cvtLayer, imgFile, buffering);
...
void DrawBgEffectDetail(int idx, int x, int y, int dirF, int res, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i, type, imgFile;
	int drawx, drawy;
	const signed short* cPtr;
...
{
	if (icon >= ICON_BOX) {
		DrawIcon(icon, x - (float)BOXSIZE_X / 2 * zoom / 3 - (float)3 * _2X * zoom / 3, y + (float)BOXSIZE_Y * zoom / 3 - (float)12 * _2X * zoom / 3, zoom / 3, true, false, false, true, cvtDest, cvtLayer, buffering);
	}
	else if (type == 1)
		DrawEffect(EMOTICON_ZZZ0 + frame % 12, x, y, 0, false, zoom, cvtDest, cvtLayer, buffering);
	else {
		if (frame > 2) {
			frame -= 3;
			...
			switch (type) {
			default:
				DrawEffect(emoticonRate[type * 2] + frame % emoticonRate[type * 2 + 1], x, y, 0, false, zoom, cvtDest, cvtLayer, buffering);
				break;
			case EMOTICON_SCOWL:
				DrawEffect(scowlEffect[frame % 8], x, y, 0, false, zoom, cvtDest, cvtLayer, buffering);
				break;
			case EMOTICON_SWEAT:
				DrawEffect(sweatEffect[frame % 7], x, y, 0, false, zoom, cvtDest, cvtLayer, buffering);
				break;
			case EMOTICON_QUESTION:
				DrawEffect(questionEffect[frame % 10], x, y, 0, false, zoom, cvtDest, cvtLayer, buffering);
				break;
			case EMOTICON_SURPRISE:
			case EMOTICON_HEART:
			case EMOTICON_HAPPY:
				DrawEffect(emoticonRate[type * 2] + surpriseEffect[frame % 5], x, y, 0, false, zoom, cvtDest, cvtLayer, buffering);
				break;
...
void PlayerDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i;
	...
	if (pObj->levelUpFrame && pObj->levelUpFrame < 11)
		DrawPlayer(pObj, 2000 - 1 + LEVELUP_BACK0 + pObj->levelUpFrame, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);

	else if (pObj->statUpFrame && pObj->statUpFrame < 11)
		DrawPlayer(pObj, 2000 - 1 + LEVELUP_BACK0 + pObj->statUpFrame, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);
	...
	if (pObj->invincible / 2 % 2 == 0 || pObj->levelUpFrame == 0) {
		...
		//플레이어
		DrawPlayer(pObj, pObj->motion, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);
		...
		for (i = 0; i < TOTALBUFF; i++) {
			if (pObj->buff[i] > 0) {
				if (i < TOTALPLAYERBUFF) {
					...
					DrawPlayer(pObj, pObj->motion, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, 0, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);
					SetAlpha(tempAlpha);
				}
				else {
					int bFrame;
					...
					default:
						if (pObj->buff[i] % 6 != 5)
							DrawEffect(BUFF_DMG0 + bFrame % 6, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);
						break;
					...
					case INC_MAGIC_ARENA:
						DrawEffect(BUFF_ARMOR0 + bFrame % 12, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);
						break;
					}
	...
	if (pObj->canMagnet) {
		if (pObj->magnet)
			DrawEffect(MAGNET_ACTIVE0 + robin.playtime % 5, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);
		else
			DrawEffect(MAGNET_DEACTIVE0 + robin.playtime % 12, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);
	}

	//플레이어 피격효과
	//무적시간
	if (pObj->attackedFrame > ATTACKEDFRAME + 1)
		DrawEffect(1000 + 10 - pObj->attackedFrame, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);

	//마나회복효과
	if (pObj->mpRestore)
		DrawEffect(EFFECT_RESTORE0 - 1 + pObj->mpRestore, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);

	//체력회복효과
	if (pObj->hpRestore)
		DrawEffect(EFFECT_RESTORE0 - 1 + pObj->hpRestore, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);

	//상태이상 회복효과
	if (pObj->statusRestore)
		DrawEffect(EFFECT_CURE0 - 1 + pObj->statusRestore, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);

	//마나흡수 효과
	if (pObj->mpDrain)
		DrawEffect(EFFECT_DRAIN0 - 1 + pObj->mpDrain, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);

	//체력 흡수 효과
	if (pObj->hpDrain)
		DrawEffect(EFFECT_CURE0 - 1 + pObj->hpDrain, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);

	if (pObj->hpDrain) {
		SetBlend(16, 0xFF0000);
		DrawEffect(EFFECT_DRAIN0 - 1 + pObj->hpDrain, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);
	}
	...
	//레벨업 효과
	if (pObj->levelUpFrame && pObj->levelUpFrame < 11)
		DrawPlayer(pObj, 2000 - 1 + LEVELUP_FRONT0 + pObj->levelUpFrame, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);
	else if (pObj->statUpFrame && pObj->statUpFrame < 11)
		DrawPlayer(pObj, 2000 - 1 + LEVELUP_FRONT0 + pObj->statUpFrame, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);

}

void NormalDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	InitMotion(pObj);
	DrawPlayer(pObj, pObj->motion, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);
}

void BulletLaserDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	NormalDraw(pObj, cvtDest, cvtLayer, buffering);
	...
	if (pObj->status) {
		...
		//we.bmp
		DrawImage(20 * _2X, 18 * _2X, 0 * _2X, 121 * _2X, xOffset + xx - rx - (float)(16 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - (yy - (float)(16 * _2X) * pObj->zoom) - ry, false, false, false, true, false, pObj->zoom, sprite[WEAPON_EFFECT_IMG], cvtDest, cvtLayer, WEAPON_EFFECT_IMG, buffering);
	}
}

void BulletGuidedDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	InitMotion(pObj);
...
void BulletSateliteDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i, tempMotion;
	int tempAlpha = m_lgrpAlpha;
	...
	if (pObj->frame > 24) {
		...
		for (i = pObj->y; i >= -96 * _2X * pObj->zoom * 2; i -= 96 * _2X * pObj->zoom * 2)
			DrawPlayer(pObj, 1000 + tempMotion, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (i - OBJIMGGAP) - ry, 0, pObj->zoom * 2, false, false, false, cvtDest, cvtLayer, buffering);
	...
	SetSectionClip(DX - (float)64 * _2X * pObj->zoom * 2, DY, (float)64 * _2X * pObj->zoom * 2, (float)64 * _2X * pObj->zoom * 2, buffering);
	DrawPlayer(pObj, 1000 + satelliteShotMotion[Min(24 + 36 * 3 - 1, pObj->frame)], xOffset + DX - 2 * xOffset - (float)64 * _2X * pObj->zoom * 2, DY, 0, pObj->zoom * 2,
		false, false, false, cvtDest, cvtLayer, buffering);
...
void BulletHealDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	if (pObj->frame > 1)
		DrawPlayer(pObj, pObj->motion, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, 0, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);
}

void BulletBoomerangDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	if ((pObj->attack == MAXX_SKILL_CAMPING || pObj->attack == MAXX_SKILL_HORMING) && pObj->icon != 0)
		DrawPlayer(pObj, pObj->motion + 4 + ((robin.playtime + 3) % 4), pObj->icon - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->name - OBJIMGGAP) - ry, pObj->dirF, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);

	DrawPlayer(pObj, pObj->motion + (robin.playtime % 4), xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - OBJIMGGAP) - ry, pObj->dirF, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);
}

void BulletItemDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	SetAlpha(10);
	DrawIcon(pObj->icon, xOffset - rx + (float)(pObj->jumpFrame * _2X - 8 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->attackFrame * _2X - OBJIMGGAP - 8 * _2X) - ry, pObj->zoom, false, false, false, true, cvtDest, cvtLayer, buffering);
	SetAlpha(21);
	DrawIcon(pObj->icon, xOffset - rx + (float)(pObj->hp * _2X - 8 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->maxhp * _2X - OBJIMGGAP - 8 * _2X) - ry, pObj->zoom, false, false, false, true, cvtDest, cvtLayer, buffering);
	SetAlpha(32);
...
{
	DrawFrame(x, y, (float)(36 * _2X) * zoom, (float)(36 * _2X) * zoom, FRAME_SHOPBALLOON, cvtDest, cvtLayer, buffering);
	...
	ShadowImage(40 * _2X, 16 * _2X, 26 * _2X, 1 * _2X, x + (float)(36 * _2X / 2 - 20 * _2X) * zoom, y + (float)(-32 * _2X + 8 * _2X) * zoom, SHADOW_IMG, zoom, cvtDest, cvtLayer, buffering);
	...
	if (enemyIdx < ENEMY_SNAIL)
		DrawPlayerCostume(
			...
			x + (float)(36 * _2X / 2) * zoom, y + (float)(-32 * _2X) * zoom, RIGHT, false, zoom * HOUSEPLAYERZOOM, cvtDest, cvtLayer, buffering
		);
	...
	if (star)
		DrawStar(ICON_STAR, x + (float)(18 * _2X) * zoom, y + (float)(ITEMICONSIZE / 2 * 0.6f + ITEMICONSIZE * 0.6f / 2) * zoom, star, star, star, CENTER, true, 0.6f * zoom, cvtDest, cvtLayer, buffering);

	if (lv)
		DrawLv(lv, x + (float)(20 * _2X) * zoom, y - (float)36 * _2X * zoom, 0.8f * zoom, CENTER, cvtDest, cvtLayer, buffering);
...
void EnemyDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int temp = 0;
	int i;
	int obj = GetObjFromPtr(pObj);
	...
	if (pObj->levelUpFrame > 0 && pObj->levelUpFrame < 11) {
		...
		//ao[NPC].x = pObj->x;
		//ao[NPC].y = pObj->y;
		DrawPlayer(&ao[NPC], 2000 - 1 + LEVELUP_BACK0 + pObj->levelUpFrame, pObj->x, STATUSWIN_Y + (rh - 4) * TSIZE - pObj->y - ry + OBJIMGGAP, false, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);
	}
	...
	if ((pObj->type == ENEMY_CASTLE_BOSS1
		...
		if (disY < ao[pObj->mom].y) {
			while (disY < ao[pObj->mom].y - 18 * _2X) {
				//c48.bmp
				DrawImage(20 * _2X, 20 * _2X, 73 * _2X, 52 * _2X, xOffset - rx + disX - (float)(10 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - (disY - (float)20 * _2X * pObj->zoom) - ry, false, false, false, false, false, pObj->zoom, sprite[MONSTER_IMG + pObj->cmf], cvtDest, cvtLayer, MONSTER_IMG + pObj->cmf, buffering);
	...
	if (pObj->levelUpFrame > 0 && pObj->levelUpFrame < 11) {
		...
		//ao[NPC].x = pObj->x;
		//ao[NPC].y = pObj->y;
		DrawPlayer(&ao[NPC], 2000 - 1 + LEVELUP_FRONT0 + pObj->levelUpFrame, pObj->x, STATUSWIN_Y + (rh - 4) * TSIZE - pObj->y - ry + OBJIMGGAP, false, pObj->zoom, false, false, false, cvtDest, cvtLayer, buffering);
		DrawEffect(EFFECT_LEVELUP_TEXT0 - 1 + (pObj->levelUpFrame < 11 ? pObj->levelUpFrame : Max(11, pObj->levelUpFrame - 5)), pObj->x, STATUSWIN_Y + (rh - 4) * TSIZE - pObj->y - ry + OBJIMGGAP - (float)(16 * _2X) * pObj->zoom, 0, false, pObj->zoom, cvtDest, cvtLayer, buffering);
...
void CloudDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i, x, y, w;
	...
	case PO_C24_A4:
		for (i = 0; i <= height; i += TSIZE) {
			...
			DrawImage(w, 16 * _2X, x, y, xOffset + pObj->x - 10 * _2X - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - 15 * _2X + (16 * _2X * (i / TSIZE))) - ry, false, false, false, false, false, 1.0f, sprite[MONSTER_IMG + pObj->cmf], cvtDest, cvtLayer, MONSTER_IMG + pObj->cmf, buffering);
		}

		SetAlpha(12);
		//c24.bmp
		DrawImage(32 * _2X, 18 * _2X, 56 * _2X, 68 * _2X, xOffset + pObj->x - 15 * _2X - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - 15 * _2X + (16 * _2X * ((i - (i - height)) / TSIZE))) - ry, false, false, false, false, false, 1.0f, sprite[MONSTER_IMG + pObj->cmf], cvtDest, cvtLayer, MONSTER_IMG + pObj->cmf, buffering);
		SetAlpha(32);
		break;
	case PO_C24_A5:
		for (i = 0; i <= height; i += TSIZE) {
		...
		//c24.bmp
		DrawImage(28 * _2X, 28 * _2X, 42 * _2X, 20 * _2X, xOffset + pObj->x - 14 * _2X - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - 25 * _2X) - ry, false, false, false, false, false, 1.0f, sprite[MONSTER_IMG + pObj->cmf], cvtDest, cvtLayer, MONSTER_IMG + pObj->cmf, buffering);
		break;
	case PO_C24_A6:
		SetAlpha(12);
		...
		for (i = 32 * _2X; i <= height; i += 16 * _2X)
			//c24.bmp
			DrawImage(6 * _2X, 32 * _2X, 0 * _2X, 35 * _2X, xOffset + pObj->x - 8 * _2X - rx + Random(5) * _2X, STATUSWIN_Y + (rh - 4) * TSIZE - (pObj->y - 47 * _2X + (32 * _2X * (i / (32 * _2X)))) - ry, false, false, false, false, false, 1.0f, sprite[MONSTER_IMG + pObj->cmf], cvtDest, cvtLayer, MONSTER_IMG + pObj->cmf, buffering);
...
void SpiderDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i = 0, h, y;        //w = 64
	...
	case PO_C27_D_LAY2:
		...
		while (i < 6) {
			if (pObj->dirF == RIGHT || i != 0) {
				...
				//c27.bmp
				DrawImage(64 * _2X, h, 0 * _2X, 73 * _2X, xOffset + pObj->x - rx + (float)(64 * _2X * i * DIR(pObj->dirF)) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - y, false, false, false, false, false, pObj->zoom, sprite[MONSTER_IMG + pObj->cmf], cvtDest, cvtLayer, MONSTER_IMG + pObj->cmf, buffering);
			}
	...
	case PO_C27_D_LAY3:
		...
		while (i < 6) {
			if (pObj->dirF == RIGHT || i != 0) {
				...
				//c27.bmp
				DrawImage(64 * _2X, h, 0 * _2X, 103 * _2X, xOffset + pObj->x - rx + (float)(64 * _2X * i * DIR(pObj->dirF)) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - y, false, false, false, false, false, pObj->zoom, sprite[MONSTER_IMG + pObj->cmf], cvtDest, cvtLayer, MONSTER_IMG + pObj->cmf, buffering);
			}
...
void SkelDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	if (pObj->status == 1)
...
void MachineDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	DrawCmf(pObj, false, pObj->zoom, false, cvtDest, cvtLayer, buffering);
}

void LaserDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i;
	...
	for (i = 0; i <= pObj->x / (float)(32 * _2X * zoom) + 4; i++) {
		...
		DrawCmf(pObj, false, zoom, false, cvtDest, cvtLayer, buffering);
	}
}

void GhostDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
...
void VanishDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	if (pObj->frame < FPS / 2) {
		...
		if (pObj->frame >= FPS / 3 - 2 * 4)
			DrawEffect((pObj->frame - (FPS / 3 - 2 * 4)) / 2 + HIT_DEAD1, xOffset + PxlLeft(pObj) + pObj->cx / 2 - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (PxlUp(pObj) + pObj->cy / 2 - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);
		else
			DrawEffect(HIT_ITEM_LARGE0 + 1000 - 1 + pObj->frame % 5, xOffset + PxlLeft(pObj) + pObj->cx / 2 - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (PxlUp(pObj) + pObj->cy / 2 - OBJIMGGAP) - ry, pObj->dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);
	}


}

void RegenDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int obj = GetObjFromPtr(pObj);
	//전투신 처음의 등장신에서는 
	if (obj < PLAYERALL) {
...
void NeutralDraw(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i;
	unsigned char* maPtr;
	float gapX, gapY;
	...
	case OBJ_DOOR:
		...
		DrawImage(16 * _2X, pObj->cy, pObj->icon * 16 * _2X, 48 * _2X - pObj->cy, xOffset + pObj->x - rx + pObj->cpx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-OBJIMGGAP + 1 * _2X) * pObj->zoom), false, false, false, false, false, pObj->zoom, sprite[DOOR_IMG], cvtDest, cvtLayer, DOOR_IMG, buffering);
		DrawImage(16 * _2X, pObj->cy, pObj->icon * 16 * _2X, 48 * _2X - pObj->cy, xOffset + pObj->x - rx + pObj->cpx + (float)(16 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-OBJIMGGAP + 1 * _2X) * pObj->zoom), true, false, false, false, false, pObj->zoom, sprite[DOOR_IMG], cvtDest, cvtLayer, DOOR_IMG, buffering);

		maPtr = &mapArray[(pObj->y / TSIZE) * mapData[1] + (pObj->x - 8 * _2X) / TSIZE];

		//t0.bmp
		DrawImage(TSIZE, TSIZE, (*maPtr - 1) % MAXTILE * TSIZE, (*maPtr - 1) / MAXTILE * TSIZE, xOffset - rx + (pObj->x - (float)(8 * _2X) * pObj->zoom) / TSIZE * TSIZE, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y / TSIZE * TSIZE), false, false, false, false, false, pObj->zoom, sprite[MAP_TILE_IMG + mapData[7]], cvtDest, cvtLayer, MAP_TILE_IMG + mapData[7], buffering);
		DrawImage(TSIZE, TSIZE, (*maPtr - 1) % MAXTILE * TSIZE, (*maPtr - 1) / MAXTILE * TSIZE, xOffset - rx + (pObj->x + (float)(8 * _2X) * pObj->zoom) / TSIZE * TSIZE, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y / TSIZE * TSIZE), false, false, false, false, false, pObj->zoom, sprite[MAP_TILE_IMG + mapData[7]], cvtDest, cvtLayer, MAP_TILE_IMG + mapData[7], buffering);

		if (pObj->mainFrame) {
			//바닥먼지
			DrawNeutral(OBJ_DOORDUST0 - 1 + pObj->mainFrame, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - OBJIMGGAP), 0, pObj->zoom, cvtDest, cvtLayer, buffering);
			DrawNeutral(OBJ_DOORDUST0 - 1 + pObj->mainFrame, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - OBJIMGGAP), 1, pObj->zoom, cvtDest, cvtLayer, buffering);
		}

		if (pObj->jumpFrame) {
			//바닥먼지
			DrawNeutral(OBJ_DOORDUST7 - 1 + pObj->jumpFrame, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - (float)OBJIMGGAP * pObj->zoom), 0, pObj->zoom, cvtDest, cvtLayer, buffering);
			DrawNeutral(OBJ_DOORDUST7 - 1 + pObj->jumpFrame, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - (float)OBJIMGGAP * pObj->zoom), 1, pObj->zoom, cvtDest, cvtLayer, buffering);
		}
		break;
	case OBJ_REPULSION:
		SetAlpha(32 - Abs(robin.playtime % 16 - 8) * 4);
		//bg11.bmp
		DrawImage(12 * _2X, 12 * _2X, 46 * _2X, 28 * _2X, xOffset + pObj->x - rx - (float)(6 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-OBJIMGGAP - 6 * _2X) * pObj->zoom), false, false, false, false, false, pObj->zoom, sprite[MAP_OBJ_IMG], cvtDest, cvtLayer, MAP_OBJ_IMG, buffering);
		break;
	case OBJ_ITEM:
		...
		*/
		...
		if (pObj->def == ITEM_GOLD)
			DrawIcon(ICON_GOLD + (frame % GOLDICONFRAME), xOffset + pObj->x - (float)(ITEMICONSIZE / 2) * pObj->zoom - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-ITEMICONSIZE - OBJIMGGAP) * pObj->zoom), pObj->zoom, false, false, false, true, cvtDest, cvtLayer, buffering);
		else
			DrawIcon(GetItemIcon(pObj->def, pObj->apx, pObj->apy), xOffset + pObj->x - (float)(8 * _2X) * pObj->zoom - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-16 * _2X - OBJIMGGAP) * pObj->zoom), pObj->zoom, false, false, false, true, cvtDest, cvtLayer, buffering);

		UnSetBlend();

		if (pObj->mainFrame % 20 < 4)
			DrawNeutral(OBJ_ITEM0 + pObj->mainFrame % 4, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - (float)(OBJIMGGAP)*pObj->zoom), 0, pObj->zoom, cvtDest, cvtLayer, buffering);
	...
	case OBJ_SOUL:
		if (pObj->status)
			DrawEffect(EFFECT_SLOT0 + pObj->frame, pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y), 0, false, pObj->zoom, cvtDest, cvtLayer, buffering);
		break;
	case OBJ_WARP:
		if (pObj->status && !isDemo) {
			if (pObj->status < 4)
				DrawEffect(EFFECT_DEMOTALK_ARROW0 + Abs(4 - robin.playtime % 8), xOffset + pObj->x - rx + pObj->cx / 2, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + pObj->cy - (float)(11 * _2X) * pObj->zoom), 0, false, pObj->zoom, cvtDest, cvtLayer, buffering);
			else
				DrawEffect(EFFECT_INSIDE_ARROW0 + Abs(4 - robin.playtime % 8), xOffset + pObj->x - rx - (float)(8 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + pObj->cy / 3), 1, false, pObj->zoom, cvtDest, cvtLayer, buffering);
		}
		break;
	case OBJ_PUSH:
		if (pObj->etc != PUSH_NORMAL && pObj->str == 0)
			DrawNeutral(OBJ_PUSHDEST0, xOffset + pushArray[pObj->etc * 2] * TSIZE - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - pushArray[pObj->etc * 2 + 1] * TSIZE, pObj->dirF, pObj->zoom, cvtDest, cvtLayer, buffering);
	...
	case OBJ_BALLBLOCK:
		...
		DrawNeutral(pObj->motion, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - (float)(OBJIMGGAP)*pObj->zoom), pObj->dirF, pObj->zoom, cvtDest, cvtLayer, buffering);
		SetAlpha(32);

		if (pObj->status && pObj->motion < *(maPtr + 9) + 3)
			DrawEffect(pObj->motion - (*(maPtr + 9)), xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-TSIZE - OBJIMGGAP) * pObj->zoom), 1 - ao[raidPlayer].dirF, false, pObj->zoom, cvtDest, cvtLayer, buffering);
		break;
	case OBJ_MAGMA:
		if (pObj->status) {
			//마그마 중단부분
			for (i = 0; i < pObj->jumpFrame - 1; i++)
				DrawNeutral(OBJ_MAGMA6, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - (float)(i * 16 * _2X) * pObj->zoom), 0, pObj->zoom, cvtDest, cvtLayer, buffering);

			//마그마 하단부분
			DrawNeutral(OBJ_MAGMA0 + (robin.playtime % 3), xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y), 0, pObj->zoom, cvtDest, cvtLayer, buffering);

			if (pObj->jumpFrame)
				//마그마 상단부분
				DrawNeutral(OBJ_MAGMA3 + (robin.playtime % 3), xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - (float)(i * 16 * _2X) * pObj->zoom), 0, pObj->zoom, cvtDest, cvtLayer, buffering);
		}
		break;
	case OBJ_BRIDGE:
		for (i = 0; i < 18 - pObj->mainFrame / 2; i += 6) {
			if (i / 6 == (18 - pObj->mainFrame / 2) / 6) {
				DrawNeutral(pObj->motion + (pObj->mainFrame / 2 % 6) + pObj->str * 6, xOffset + pObj->x - rx + (float)(i / 6 * 32 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - ry - pObj->y, 0, pObj->zoom, cvtDest, cvtLayer, buffering);
				DrawNeutral(pObj->motion + (pObj->mainFrame / 2 % 6) + pObj->str * 6, xOffset + pObj->x + pObj->cx - rx - (float)(i / 6 * 32 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - ry - pObj->y, 1, pObj->zoom, cvtDest, cvtLayer, buffering);
			}
			else {
				DrawNeutral(pObj->motion + pObj->str * 6, xOffset + pObj->x - rx + (float)(i / 6 * 32 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - ry - pObj->y, 0, pObj->zoom, cvtDest, cvtLayer, buffering);
				DrawNeutral(pObj->motion + pObj->str * 6, xOffset + pObj->x + pObj->cx - rx - (float)(i / 6 * 32 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - ry - pObj->y, 1, pObj->zoom, cvtDest, cvtLayer, buffering);
			}
		}
		break;
	case OBJ_STALACTITE:
		...
		DrawBgEffect(pObj->motion, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y), 0, pObj->zoom, cvtDest, cvtLayer, buffering);
		break;
	case OBJ_SPIKE:
		if (pObj->etc == 4)
			SetAlpha(pObj->mainFrame);

		DrawNeutral(pObj->motion, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y), pObj->dirF, pObj->zoom, cvtDest, cvtLayer, buffering);
		break;
	case OBJ_MAGNET:
		DrawNeutral(OBJ_MAGNET4 - Abs(robin.playtime % 8 - 4), xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - OBJIMGGAP), pObj->dirF, pObj->zoom, cvtDest, cvtLayer, buffering);

		if (pObj->etc)
			DrawEffect(MAGNET_ACTIVE0 + (robin.playtime % 5), xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - OBJIMGGAP + 13), 0, false, pObj->zoom, cvtDest, cvtLayer, buffering);
		break;
	case OBJ_SLED:
		if (pObj->etc == 3)
			SetAlpha(pObj->mainFrame);
	...
	case OBJ_CATCHER:
		if (pObj->status) {
			SetAlpha(24);
			DrawEffect(BUFF_DMG0 + robin.playtime % 5, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + 8 * _2X), 0, false, pObj->zoom, cvtDest, cvtLayer, buffering);
			SetAlpha(32);
		}
		break;
	case OBJ_CYCLON:
		if (pObj->status)
			DrawNeutral(OBJ_CYCLON7 + robin.playtime % 3, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y), 0, pObj->zoom, cvtDest, cvtLayer, buffering);
		else
			DrawNeutral(OBJ_CYCLON0 + robin.playtime % 7, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y), 0, pObj->zoom, cvtDest, cvtLayer, buffering);
		break;
	case OBJ_FLOWER:
		DrawIcon(198, xOffset + pObj->x - 8 * _2X - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - 16 * _2X - OBJIMGGAP), pObj->zoom, false, false, false, true, cvtDest, cvtLayer, buffering);
		break;
	case OBJ_GOLD:
		UnSetBlend();
		switch (mapData[7]) {
		case MAPTYPE_TOLEM:	//톨렘마을 바깥//0
			DrawImage(108 * _2X, 80 * _2X, 0 * _2X, 0 * _2X, xOffset + pObj->x - rx - (float)(6 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-OBJIMGGAP - 6 * _2X) * pObj->zoom), false, false, false, false, false, pObj->zoom, sprite[MAP_OBJ_IMG + mapData[7]], cvtDest, cvtLayer, MAP_OBJ_IMG + mapData[7], buffering);

			break;
		case MAPTYPE_TOLEMINSIDE:	//톨렘마을 집안//1
			break;
		case MAPTYPE_SWAMP:	//톨레아 습지//2
			DrawImage(108 * _2X, 80 * _2X, 0 * _2X, 0 * _2X, xOffset + pObj->x - rx - (float)(56 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-OBJIMGGAP - 112 * _2X) * pObj->zoom), false, false, false, false, false, pObj->zoom, sprite[MAP_OBJ_IMG + mapData[7]], cvtDest, cvtLayer, MAP_OBJ_IMG + mapData[7], buffering);
			DrawImage(66 * _2X, 32 * _2X, 0 * _2X, 80 * _2X, xOffset + pObj->x - rx - (float)(56 * _2X - 24 * _2X) * pObj->zoom, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-OBJIMGGAP - 32 * _2X) * pObj->zoom), false, false, false, false, false, pObj->zoom, sprite[MAP_OBJ_IMG + mapData[7]], cvtDest, cvtLayer, MAP_OBJ_IMG + mapData[7], buffering);
			...
			for (i = 0; i < 4; i++) {
				...
				DrawIcon(ICON_GOLD + frame % GOLDICONFRAME, xOffset + pObj->x - rx - (float)(56 * _2X) * pObj->zoom + gapX * pObj->zoom - (float)ITEMICONSIZE * iconZoom * pObj->zoom / 2, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-OBJIMGGAP - 112 * _2X) * pObj->zoom) + gapY * pObj->zoom + (float)ITEMICONSIZE * iconZoom * pObj->zoom / 2, pObj->zoom * iconZoom, COLOR_WHITE, false, true, true, cvtDest, cvtLayer, buffering);
			}
			//골드백
			iconZoom = 0.8f;
			for (i = 0; i < 3; i++) {
				...
				DrawIcon(ICON_GOLDBAG, xOffset + pObj->x - rx - (float)(56 * _2X) * pObj->zoom + gapX * pObj->zoom - (float)ITEMICONSIZE * iconZoom * pObj->zoom / 2, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-OBJIMGGAP - 112 * _2X) * pObj->zoom) + gapY * pObj->zoom + (float)ITEMICONSIZE * iconZoom * pObj->zoom / 2, pObj->zoom * iconZoom, COLOR_WHITE, false, true, true, cvtDest, cvtLayer, buffering);
			}
			//방패
			iconZoom = 0.9f;
			for (i = 0; i < 2; i++) {
				...
				DrawIcon(ICON_SHIELD, xOffset + pObj->x - rx - (float)(56 * _2X) * pObj->zoom + gapX * pObj->zoom - (float)ITEMICONSIZE * iconZoom * pObj->zoom / 2, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-OBJIMGGAP - 112 * _2X) * pObj->zoom) + gapY * pObj->zoom + (float)ITEMICONSIZE * iconZoom * pObj->zoom / 2, pObj->zoom * iconZoom, COLOR_WHITE, false, true, true, cvtDest, cvtLayer, buffering);
			}
			//하트
			...
			for (i = 0; i < 1; i++) {
				...
				DrawIcon(ICON_HEART, xOffset + pObj->x - rx - (float)(56 * _2X) * pObj->zoom + gapX * pObj->zoom - (float)ITEMICONSIZE * iconZoom * pObj->zoom / 2, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y + (float)(-OBJIMGGAP - 112 * _2X) * pObj->zoom) + gapY * pObj->zoom + (float)ITEMICONSIZE * iconZoom * pObj->zoom / 2, pObj->zoom * iconZoom, COLOR_WHITE, false, true, true, cvtDest, cvtLayer, buffering);
			}
			break;
		case MAPTYPE_VALLEY:	//금단의 계곡//3
			break;
		case MAPTYPE_ATLANTICE:	//아틀란티스//4
			break;
		case MAPTYPE_SEWAGE:	//아델라인 지하도//5
			break;
		case MAPTYPE_CASTLE:	//아델라인 성//6
			break;
		case MAPTYPE_PLAIN:	//아델 평원//7
			break;
		case MAPTYPE_ELF:	//벨로네 마을//8
			break;
		case MAPTYPE_FLAME:	//홍염의 대지//9
			break;
		case MAPTYPE_FROST:	//유구한 빙원//10
			break;
		case MAPTYPE_THUNDER:	//뇌전의 공방//11
			break;
		case MAPTYPE_LIGHT:	//황혼의 성역//12
			break;
		case MAPTYPE_GOLEMVALLEY:	//골렘의 협곡//13
			break;
		case MAPTYPE_DARKNESS:	//칠흑의 심연//14
			break;
		case MAPTYPE_DRAGON:	//드래곤의 무덤//15
			break;
		case MAPTYPE_GHOST:	//금지된 땅//16
			break;
		case MAPTYPE_DEVILCASTLE:	//마왕성//17
			break;
		case MAPTYPE_SPACE:	//우주공간//18
			break;
		}
	...
	default:
		...
		if (pObj->type < OBJ_LEVER) {
			if (pObj->motion != 0xFF) {
				...
				DrawImage(*(usPtr + 2), *(usPtr + 3), *usPtr, *(usPtr + 1), xOffset + pObj->x - rx + pObj->cpx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - OBJIMGGAP + pObj->cpy + neutralOffset[pObj->motion] - ((pObj->type == OBJ_ITEM && pObj->apx == ITEM_QUEST_REEF) ? 12 * _2X : 0 * _2X) * pObj->zoom), pObj->dirF, false, false, false, false, pObj->zoom, sprite[imgFile], cvtDest, cvtLayer, imgFile, buffering);
			...
			DrawNeutral(pObj->motion, xOffset + pObj->x - rx, STATUSWIN_Y + (rh - 4) * TSIZE - ry - (pObj->y - OBJIMGGAP), pObj->dirF, pObj->zoom, cvtDest, cvtLayer, buffering);
		break;
	}
...
void DrawShadowPlayer(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i, j, k;
	...
	for (i = (pObj->x - 12 * _2X) / TSIZE; i <= (pObj->x + 12 * _2X) / TSIZE; i++) {
		//각 위치에 따라 밑으로 내려가면서 바닥이랑 만나는 곳을 계산한다.
		for (j = pObj->y / TSIZE, tHeight = 0; j < rh - 1; j++) {
			...
			if (mapInfoArray[mapInfoOff + j * rw + i] >= TILE_BLOCK || (pObj->inTile == GROUND && pObj->onWater > 0 && mapInfoArray[mapInfoOff + j * rw + i] >= TILE_WATER) || j == rh - 2) {
				...
				//그림자 전체의 우측 경계면 : (pObj->x + 12)
				//현재 타일의 좌측 경계면 : i * TSIZE;
				//현재 타일의 우측 경계면 : (i + 1) * TSIZE
				//그려야할 지점의 좌측 경계면 : max(pObj->x - 12, i * TSIZE)
				//그려야할 지점의 우측 경계면 : min(pObj->x + 12, (i + 1) * TSIZE)
				...
				//블럭오브젝트에 그림자가 그려져야 하는 경우
				if (shadowBlockY < shadowTileY) {
					//블럭오브젝트 왼쪽 끝이 그려야 할 그림자 왼쪽 끝과 같다면
					if (bcLeft == sLeft) {
						pObj->nHeight = bHeight;
						ShadowImage(bcRight - sLeft, 16 * _2X, offset, 0, xOffset + sLeft - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (shadowBlockY - 14 * _2X) - ry, COMMON_IMG, pObj->zoom, cvtDest, cvtLayer, buffering);

						//블럭오브젝트 오른쪽 끝이 그려야 할 그림자 오른쪽 끝과 다르다면 타일쪽에 나머지부분을 한번 더 그려준다.
						if (bcRight != sRight) {
							pObj->nHeight = tHeight;
							ShadowImage(sRight - bcRight, 16 * _2X, offset + bcRight - sLeft, 0, xOffset + bcRight - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (shadowTileY - 14 * _2X) - ry, COMMON_IMG, pObj->zoom, cvtDest, cvtLayer, buffering);
						}
					}
					//다르다면
					else {
						//우선 타일쪽에 왼쪽부분을 한번 그리고
						pObj->nHeight = tHeight;
						ShadowImage(bcLeft - sLeft, 16 * _2X, offset, 0, xOffset + sLeft - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (shadowTileY - 14 * _2X) - ry, COMMON_IMG, pObj->zoom, cvtDest, cvtLayer, buffering);

						//블럭오브젝트쪽에 나머지 부분을 그린다
						pObj->nHeight = bHeight;
						ShadowImage(bcRight - bcLeft, 16 * _2X, offset + bcLeft - sLeft, 0, xOffset + bcLeft - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (shadowBlockY - 14 * _2X) - ry, COMMON_IMG, pObj->zoom, cvtDest, cvtLayer, buffering);
					}
				}
				//타일에 그려져야 하는 경우
				else if (shadowTileY < 10000) {
					pObj->nHeight = tHeight;
					ShadowImage(sRight - sLeft, 16 * _2X, offset, 0, xOffset + sLeft - rx, STATUSWIN_Y + (rh - 4) * TSIZE - (shadowTileY - 14 * _2X) - ry, COMMON_IMG, pObj->zoom, cvtDest, cvtLayer, buffering);
				}
...
void DrawShadowCommon(OBJECT* pObj, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	int i, j;
	int offset1, offset2;
	int sLeft;
	...
	for (i = sLeft / TSIZE; i <= sRight / TSIZE; i++) {
		...
		for (j = PxlDown(pObj) / TSIZE; j < rh - 1; j++) {
			...
			if (mapInfoArray[mapInfoOff + j * rw + i] >= TILE_BLOCK) {
				...
				//그림자 전체의 우측 경계면 : sRight
				//현재 타일의 좌측 경계면 : i * TSIZE;
				//현재 타일의 우측 경계면 : (i + 1) * TSIZE
				//그려야할 지점의 좌측 경계면 : max(sLeft, i * TSIZE)
				//그려야할 지점의 우측 경계면 : min(sRight, (i + 1) * TSIZE)
				//그려야 할 그림자의 폭 : min(sRight, (i + 1) * TSIZE) - max(sLeft, i * TSIZE)
				if (sLeft >= i * TSIZE)
					//왼쪽
					ShadowImage((i + 1) * TSIZE - sLeft, 16 * _2X, 24 * _2X, 0, xOffset + (float)sLeft - rx, (STATUSWIN_Y + (rh - 4) * TSIZE - y), COMMON_IMG, pObj->zoom, cvtDest, cvtLayer, buffering);
				else if (sRight <= (i + 1) * TSIZE)
					//오른쪽
					ShadowImage(sRight - i * TSIZE, 16 * _2X, 64 * _2X - (sRight - i * TSIZE), 0, xOffset + (float)i * TSIZE - rx, (STATUSWIN_Y + (rh - 4) * TSIZE - y), COMMON_IMG, pObj->zoom, cvtDest, cvtLayer, buffering);
				else {
					offset1 = 0;
					offset2 = 0;
					...
					if (sLeft >= (i - 1) * TSIZE && TSIZE - (sLeft % TSIZE) < 12 * _2X) {
						...
						ShadowImage((sLeft + 12 * _2X) % TSIZE, 16 * _2X, 36 * _2X - offset1, 0, xOffset + (float)i * TSIZE - rx, (float)(STATUSWIN_Y + (rh - 4) * TSIZE - y), COMMON_IMG, pObj->zoom, cvtDest, cvtLayer, buffering);
					}

					if (sRight < (i + 2) * TSIZE && sRight % TSIZE < 12 * _2X) {
						//오른쪽에서 두번째
						offset2 = (i + 1) * TSIZE - (sRight - 12 * _2X);
						ShadowImage((i + 1) * TSIZE - (sRight - 12 * _2X), 16 * _2X, 52 * _2X, 0, xOffset + (float)(sRight - 12 * _2X) - rx, (float)(STATUSWIN_Y + (rh - 4) * TSIZE - y), COMMON_IMG, pObj->zoom, cvtDest, cvtLayer, buffering);
					}

					ShadowImage(TSIZE - (offset1 + offset2), 16 * _2X, 36 * _2X, 0, xOffset + (float)(i * TSIZE + offset1) - rx, STATUSWIN_Y + (float)((rh - 4) * TSIZE - y), COMMON_IMG, pObj->zoom, cvtDest, cvtLayer, buffering);
				}
				break;
			}
