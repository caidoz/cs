...
bool Core::init()
{
	...
	//---- 콘텐츠 준비 ----
	//
	//순서가 중요하다. 아래 셋은 이 차례로만 맞다.
	//
	//  1. 팩     : 데이터를 읽어 Data/*.h 의 포인터를 팩 안쪽으로 옮긴다.
	//  2. 표     : cmfOff 같은 표를 채운다.
	//  3. CmfRead: 그 표를 읽어 cmf를 메모리에 편다.
	//
	//예전에는 3이 1보다 먼저였다. 표가 헤더에 박힌 const 배열이라 언제
	//읽어도 같았기 때문이다. 지금은 표를 런타임에 채우므로, 2보다 먼저
	//3을 하면 빈 표를 읽어 죽는다.
	//
	//CDN 갱신은 여기서 안 한다. AppDelegate 가 부팅 맨 앞에서 "이미 받아둔
	//것"을 넣어줬고(ContentBoot), 새로 받는 일은 아래에서 딴 실에 맡긴다.
	//데이터를 다 읽은 뒤에 시작해야 한다. 팩 버퍼를 읽는 중에 파일이 바뀌면
	//안 되기 때문인데, 받는 쪽은 staging 에만 쓰므로 서로 안 부딪친다.
	...
	//cmf/지도 표를 채운다. 팩을 읽었으면 팩 안쪽을, 못 읽었으면 내장본을
	//가리키게 된다. 팩 읽기가 실패해도 반드시 불러야 한다. 표가 비어 있으면
	//바로 아래 CmfRead 가 죽는다.
	CmfRelink();
	MapRelink();
	...
	//다음 판을 뒤에서 받기 시작한다. 곧바로 돌아온다.
	//받은 것은 staging 에만 쌓이고, 반영은 다음 부팅 때 한다. 그래서
	//지금 도는 게임이 쓰는 팩과 그림은 그대로다.
	ContentUpdateBegin();
...
void Core::Run(float delta) {
	...
	//상태가 바뀔 때만 한 줄 남긴다. 매 프레임 찍으면 로그가 묻힌다.
	{
		static int sSeen = -1;
		int done = 0, total = 0;
		int now = ContentPoll(&done, &total);

		if (now != sSeen) {
			sSeen = now;

			if (now == CONTENT_DOWNLOADING)
				CCLOG("Content: 받는 중 %d/%d", done, total);
			else if (now == CONTENT_READY)
				CCLOG("Content: 다 받았다. 다음에 켜면 새 콘텐츠로 시작한다");
		}
	}
