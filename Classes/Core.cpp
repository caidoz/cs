...
bool Core::init()
{
	...
	//---- 콘텐츠 준비 ----
	//
	//순서가 중요하다. 아래 넷은 이 차례로만 맞다.
	//
	//  1. 갱신   : CDN에서 바뀐 파일을 받아 쓰기 가능한 곳에 놓는다.
	//              검색 경로가 그쪽을 먼저 보므로 이후 읽기는 새 파일이다.
	//  2. 팩     : 데이터를 읽어 Data/*.h 의 포인터를 팩 안쪽으로 옮긴다.
	//  3. 표     : cmfOff 같은 표를 채운다.
	//  4. CmfRead: 그 표를 읽어 cmf를 메모리에 편다.
	//
	//예전에는 4가 2보다 먼저였다. 표가 헤더에 박힌 const 배열이라 언제
	//읽어도 같았기 때문이다. 지금은 표를 런타임에 채우므로, 3보다 먼저
	//4를 하면 빈 표를 읽어 죽는다.

	//바뀐 콘텐츠를 받는다. 못 받아도 그냥 간다. 앱에 딸려온 것을 쓴다.
	ContentUpdate();
