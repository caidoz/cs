...
#define SPEED_MIN	4
#define DAYS3 3 * 24 * 60 * 60 * 1000 // 3일
...
typedef enum _gameDef {
	...
	POISON,	//독->턴마다 독데미지를 받음
	STUN,	//기절->1턴동안 공격을 못함
	CURSE,	//저주(MP를 사용못함)->공격을 못함
	KNOCKBACK,	//밀어내기? 이건 쓸 필요가 없을듯.(제거)
	TOTALDEBUF,
	...
	DAMAGExN,	//데미지N배
#else
	HPRESTORE,//,	//여유포착 : 적 공격 회피시 HP 상승->일정턴동안 적 공격회피
#endif	
	MPRESTORE,	//냉정침착 : 데미지 입으면 MP 회복
	REFLECTION,	//리플렉션 : 한턴 데미지 반사
	...
	INC_EVASION,	//현란무도 : 일정시간 회피율 증가, 한턴 그냥 회피
#ifdef SKILL_MAXX16_ERASEALLDEBUF 
	EVASIONTOHP,	//안도의한숨 : 적 공격 회피시 HP 회복
#else
	EVASIONTOMP,	//안도의한숨 : 적 공격 회피시 MP 회복. 회피하면 체력 회복
#endif
	...
	//스킬타입
	PASSIVE = 0,//일반 패시브 스킬(현재는 안쓰고)
	ACTIVE,//일반 액티브 스킬(현재는 안쓰고)
	CREWBULLET,//총탄 공격
	SUMMON,//소환
	HEROSKILL,//히어로의 스킬을 호출
	SUMMONHERO,//주인공 빼고 다른 히어로 소환
	CREWSUMMON,//동료가 워프하여 주인공 앞에 난입한 다음에 해당 동료가 공격함.(엘케인, 라베스 등이 될 수 있음)
	...
	MENU_CASTLE,//성 관련
	MENU_SHOP,//상점 : 다이아몬드(캐시) -> 소환, 에메랄드 -> 스킬슬롯, 장비개수 등, 골드()->레벨업, 강화석()->강화, 스킬카드(), 황금열쇠(구매를 하면 쌓이는 특별)
	MENU_STARSHOP,
	MENU_FRIENDS,
	MENU_NEWS,
	MENU_GIFTS,
...
typedef enum _dailyQuest {
	...
	DAILYQUEST_OPEN_BOX_GETEQUIP = 0,//?곸옄瑜??댁뼱 ?λ퉬瑜?3踰??살쑝硫?
	DAILYQUEST_UPGRADE_CREW,//3媛쒕? ?낃렇?덉씠?쒗븯硫??섎뒗寃?
	DAILYQUEST_ATTACK_OTHERUSER,//Battle, Raid ?ы븿
	DAILYQUEST_COLLECT_GOLD,//?쇱젙湲덉븸???덉쓣 紐⑥쑝硫?
	DAILYQUEST_STAGE_TRY,//?ㅽ뀒?댁? ?꾪닾 
...
enum JumpState {
	JS_IDLE = 0,     // ?뺤?(?쒖엳??
	JS_SPINNING,     // ?먰봽 以?+ 怨듭쨷?먯꽌 怨꾩냽 援먯껜
	JS_LANDING,      // 李⑹? 援ш컙(怨듭쨷?먯꽌 援먯껜?섎떎媛� 李⑹? ?쒓컙 target ?뺤젙)
	JS_HOLD,     // 李⑹? ???곗텧 ?�湲??щ줈??
	JS_DONE          // ?뺤젙 ??怨좎젙
};
...
#define MAX_POINTS 500 * MOTIONDIV // 理쒕? 醫뚰몴 媛쒖닔
#define MAX_RADIUS 256 // 理쒕? 諛섍꼍
#define MAX_BOXRADIUS	128

#define BOSSROOMTIME	60 * 10//10遺?
