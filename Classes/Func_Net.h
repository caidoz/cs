...
enum {
	...
	//판이 끝나길 기다리는 상한. 이만큼 지나도 안 끝나면 묶기를 포기하고
	//보낸다. 한 판은 길어야 몇 초라, 여기까지 왔다면 어딘가 멎은 것이다.
	NETSAVE_TURNHOLDMAX = 60 * 30,
...
//---- 한 판을 한 덩어리로 ----
//
// 공격 한 번은 하트를 내고 시작해서 여러 프레임에 걸쳐 결과가 쌓인다.
// 그 사이에 저장이 나가면, 하트는 냈는데 결과는 아직인 상태가 서버에
// 남을 수 있다. 거기서 앱이 죽으면 값만 치르고 아무것도 못 얻는다.
//
// 그래서 판이 도는 동안에는 안 보낸다. 판이 끝나면 그때 한 번에 보낸다.
// 하트와 그 판의 결과가 같이 저장되거나, 둘 다 안 되거나 — 둘 중 하나다.
// 중간이 없다.
//
// 판이 끝나는 자리는 열여섯 군데라 일일이 부를 수가 없다. 그래서 부르는
// 쪽이 끝을 알리지 않는다. NetUpdate() 가 attackSequence 를 보고 스스로
// 판단한다. 어떤 경로로 끝나든 저절로 풀린다.
//
// 다만 판이 어딘가에서 멎으면 영영 못 보내므로 상한을 둔다. 그때는
// 묶기를 포기하고 보낸다 — 안 보내는 것보다는 낫다.
