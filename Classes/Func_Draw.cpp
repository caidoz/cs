...
//모든 이벤트 상황을 컨트롤하는 함수
//이벤트는 크게 우측에 발생하는 아이콘형 이벤트와
//현재 퀘스트에 붙는 부가 퀘스트를 확인할 수 있다.
//해당 함수에서 다양한 
void EventScheduler(void)
{
	...
	//만약 현재 메인 퀘스트가 없다면 메인퀘스트를 올려보내고
	...
	//만약 현재 PVP 퀘스트가 없다면 PVP 퀘스트를 올려보낸다.
	for (i = 0; i < MAXGAMEEVENT; i++) {
		switch (robin.gameEvent[i].type) {
		case EVENTTYPE_PVP:
			eventIdx = i;
...
void InitEventMenu(GAMEEVENT* gEvent, unsigned char type, unsigned char subType, short icon, long limitTime, int touchFunc)
{
	...
	case EVENTTYPE_DOUBLE:
		...
		break;
	case EVENTTYPE_BOSSRAID:
		gEvent->barStatus = BOSSRAID_ROYALFAMILY;//어떤 보스셋인가
		gEvent->barFrame = 0;//몇번째 스텝인가
		gEvent->value = 0;//몬스터 체력 세팅
...
void GameMenuDraw(int x, int y, float zoom, cocos2d::RenderTexture* cvtDest, cocos2d::Layer* cvtLayer, bool buffering)
{
	...
	for (i = 0; i < MENU_JOKBO; i++) {
		if (robin.openedMenu[i] == OPENED) {
			...
			//else
			switch (i) {
			case MENU_PLAY://캐릭터메뉴 : 캐릭터를 레벨업하고 스탯(STAT_STR, STAT_VIT, STAT_AGI, STAT_INT)을 찍는 부분
