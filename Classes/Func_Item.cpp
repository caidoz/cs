	...
	default:
		...
		PREFIXSUFFIX:
			...
			for (i = 0; i < prefix + suffix; i++) {
				...
				case 3:
				case 4:
					//50·¹º§ ÀÌÇÏÀÇ ¾ÆÀÌÅÛÀº ÇÏ±Þ ¶Ç´Â Áß±Þ
	...
	case HOTKEY_SKILL:
		...
		if (pObj->hotKey[0].randomFrame > 0) {
			...
			if (pObj->hotKey[0].randomFrame == 0) {
				...
				pObj->hotKey[0].random = false;//·£´ý Ãë¼Ò
...
int MakeItemId(void)
{
	//¾ÆÀÌÅÛ °íÀ¯ ¾ÆÀÌµð¸¦ ³Ö¾îÁØ´Ù.
	//°íÀ¯ ¾ÆÀÌµð °ü·Ã °ª : ¹Ð¸®¼¼ÄÁ, ÀüÈ­¹øÈ£, ¹æ¹øÈ£, ¾ÆÀÌÅÛ x, yÀ§Ä¡
	//0xAAABCDEF
	//AAA : ¹Ð¸®¼¼ÄÁ°ª
	//B : ÀüÈ­¹øÈ£ ±¹¹ø % 256
	//C : ÀüÈ­¹øÈ£ µÞÀÚ¸® % 256
...
//for¹®À» µ¹¸é¼­ ÄÝ·º¼ÇÀ» ´Ù ¸ð¾Ò´Âµ¥ ¾ÆÁ÷ º¸»óÀ» ¹ÞÁö ¾ÊÀº ÀÎµ¦½º¸¦ ´øÁø´Ù.
...
void UseRing(OBJECT* pObj, int idx)
{
	...
	switch (it->detail) {
	case ITEM_RING0:	//»ç½ÅÀÇ ¹ÝÁö(ÀúÁÖ Á¦°Å)
		useV = CURSE;
#ifdef USERINGFORALL
...
#endif
		break;
	case ITEM_RING5:	//»ýÈ¯ÀÇ ¹ÝÁö(Ã¼·Â È¸º¹)
#ifdef USERINGFORALL
		for (i = PLAYER; i < PLAYERALL; i++)
			if (ao[i].active == true && ao[i].dead == false) {
	...
	//	pObj->mpRestore = 1;
		...
		for (i = PLAYER; i < PLAYERALL; i++)
			if (ao[i].active == true && ao[i].dead == false) {
				...
				ao[i].mpRestore = 1;//¸¶³ªÈ¸º¹ È¿°úÁö¸¸ Ãß°¡Å¸°Ý È¿°ú·Î ÀÏ´Ü ¾¸
				RefreshStat(&ao[i]);
			}
#else
...
void SetActiveSkillString(int index, char* str, unsigned short width, unsigned short height)
{
	...
	for (i = 0, strPtr = &str[0]; i < 2; i++, skillValue++, strPtr = &tempStr2[0]) {
		...
		if (*skPtr2 == PASSIVE) {
			//ÆÐ½Ãºê¶ó¸é : ±âº»ÀûÀ¸·Î ´É·ÂÄ¡ ÇÏ³ª¸¸À» ÇÕ¼ºÇÑ´Ù.
			if (skillValue < 0) {
	...
	for (i = 0, strPtr = &skillBodyStr[0]; i < 2; i++, skillValue++, strPtr = &tempStr2[0]) {
		...
		if (*skPtr2 == PASSIVE) {
			...
			default:
				...
				//±âº» °ø°Ý·ÂÀÇ |r%d%%|bÀÇ À§·ÂÀ» Áö´Ï¸ç, %s
