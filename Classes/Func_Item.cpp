...
int DropItem(OBJECT* pObj, int type)
{
	...
	//오천마리 잡았을때 플레이어가 얻는 것
	//1. 오천마리에 해당하는 경험치
	//2. 2.5%(대략 100개)의 장비중 18개의 고급장비, 1.8개의 희귀장비, 0.18개의 영웅장비
	//3. 약 225000의 골드(마리당 150인 경우)
	//4. 1.4%(대략 70개)의 레시피중 7개의 고급레시피, 7개의 희귀레시피
	//5. 650개의 소모성아이템
	//6. 140개의 보석중 25개의 고급보석, 2.5개의 희귀보석, 0.25개의 영웅보석
	//7. 1150개의 재료들
...
void MakeItem(ITEM* it, int type, int lv, int grade, int detail, int set)
{
	...
	default:
		...
		if (grade == GRADE_LEGEND) {
			...
			//임시저장소에 만들어지는 아이템은 강화/옵션생성등을 하지 않는다.
#ifdef DEBUG
			...
			default:
				//case GRADE_NORMAL:
					//일반 아이템의 경우는 별도의 옵션생성을 하지 않는다.
				j = 0;
				break;
			case GRADE_SUPERIOR:

				//1~2개의 옵션을 동일한 확률로 생성
				j = Random(2) + 1;
				break;
			case GRADE_RARE:
				//3~5개의 옵션을 동일한 확률로 생성
				j = Random(4) + 2;
				break;
			case GRADE_SET:
				//세트번호 입력
		...
		PREFIXSUFFIX:
			...
			for (i = 0; i < prefix + suffix; i++) {
				...
				if (optionRange[it->option[i][0] * 6 + optionGrade[i] * 2]) {
					...
					//옵션값이 0이면 현재 달릴수 없는 옵션이니 다시 루프를 돌린다.
					i--;

					goto END;
				}

				//계열별로 적용되어야 하는 옵션인 경우 현재 무기종류에 맞추어서 옵션을 변경
...
int MakeArenaItem()
{
	...
	// 50% 확률로 장비드랍. //투기장 1층씩 내려갈때마다 0.5%씩 장비드랍률상승(100층에서는 100%로 장비드랍)
...
//단 장착된 장비와 인벤토리 장비와의 동기화가 보장되어야 함.
...
//-> 이것을 통해 현재 스킬이 활성화될 수 있는 상태인지를 확인할 수 있다.
//2. 현재 컬렉션에 모든 장비가 획득되고 장비된 상태인지
//-> 이것을 통해 현재 스킬이 사용중인 상태인지를 확인할 수 있다.
...
*/
// 우선순위와, 인벤의 번호를 얻어와서, 우선순위별로 소팅한뒤, 해당인벤의 번호아이템을 복사한다.
void OrderInven(void)
{
	...
	for (i = 0; i < robin.count; i++) {
		invenSort[i] = i;
		invenSort[TOTALINVENTORY + i] = GetOrder(&robin.inven[i], i);	//아이템의 우선순위 번호를 알아온다.
	}

	//QuickSort(invenSort, 0, robin.count);
	RadixSort(invenSort, robin.count);

	//해당 인덱스 번호를 얻어와 shop에 복사한다
...
//case 순서대로 우선순위
//장비의 경우 반지가 우선순위 가장높고, 목걸이가 가장 낮음
//장비의 경우 착용할 수 있는 장비가 우선순위 높음
//장비의 경우 detail이 높을수록 우선순위 높음
...
//현재 이큅에 있는 장비를 인벤토리에 넣고 이큅데이터를 리셋하면서 이미지를 맞춰준다.
...
void UseRing(OBJECT* pObj, int idx)
{
...
#endif
		break;
	case ITEM_RING16:	//영화의 반지(스킬 재사용 시간 감소)
#ifdef USERINGFORALL
		for (i = PLAYER; i < PLAYERALL; i++)
			if (ao[i].active == true && ao[i].dead == false)
	...
	default:
		...
		//case ITEM_RING10:	//원한의 반지(관통 증가)
		//case ITEM_RING11:	//신념의 반지(적중 증가)
		//case ITEM_RING12:	//광포의 반지(기절공격 증가)
		//case ITEM_RING13:	//승자의 반지(치명타 증가)
		//case ITEM_RING14:	//궁극의 반지(치명타 데미지 증가)
		//case ITEM_RING15:	//차원의 반지(적 방어도 무시 증가)
#ifdef USERINGFORALL
		for (i = PLAYER; i < PLAYERALL; i++)
			if (ao[i].active == true && ao[i].dead == false) {
				if (it->detail == ITEM_RING9) {//광대의 반지(37)//|e사용시 |b회피 |i%d%% |b증가
					ao[i].mpRestore = 1;
				}
				else if (it->detail == ITEM_RING10) {//원한의 반지(38)//|e사용시 |b관통공격확률 |i%d%% |b증가
					ao[i].mpDrain = 1;
				}
				else if (it->detail == ITEM_RING15) {//차원의 반지(43)//|en.사용시 |b적방어도감소 |i%d
					ao[i].hpDrain = 1;
				}
			}
#endif
		...
		break;
	case ITEM_RING17:	//행운의 반지(경험치 획득)
		useV = INC_EXP_RING;
		break;
	}
	...
	if (useV) {
		switch (it->detail) {
		case ITEM_RING3:	//해방의 반지(31)//|e사용시|i%d%%|j확률로 상태이상제거
#ifdef RINGERASEALLDEBUF
#ifdef USERINGFORALL
			for (i = PLAYER; i < PLAYERALL; i++)
				if (ao[i].active == true && ao[i].dead == false)
					if (pObj->equip[EQUIP_RING].value > Random(1000)) {
						ao[i].ringValue[ITEM_RING3] = pObj->equip[EQUIP_RING].value;//회복에 성공하면
						for (j = 0; j < TOTALDEBUF; j++)
							ao[i].debuf[j] = 0;
					}
					else
						ao[i].ringValue[ITEM_RING3] = 1;//회복에 실패하면
#else
...
#endif
			break;
		case ITEM_RING5:	//생환의 반지(33)//|e사용시 |jHP |i%d |j회복
			break;

		case ITEM_RING6:	//희생의 반지(34)//|e사용시 |i%d%%|j확률로 추가타격
		case ITEM_RING7:	//폭주의 반지(35)
		case ITEM_RING8:	//골렘의 반지(36)
		case ITEM_RING9:	//광대의 반지(37)//|e사용시 |j회피 |i%d%%|j증가
		case ITEM_RING10:	//원한의 반지(38)//|e사용시|j관통공격확률|i%d%%|j증가
		case ITEM_RING11:	//신념의 반지(39)
		case ITEM_RING12:	//광포의 반지(40)
		case ITEM_RING13:	//승자의 반지(41)
		case ITEM_RING14:	//궁극의 반지(42)
		case ITEM_RING15:	//차원의 반지(43)//|e사용시 |j적방어도감소 -|i%d
...
void SetItemString(ITEM* it, int addDetail, int addCool)
{
	...
	if (shopType == SHOPTYPE_QUICK && menuFocus < 100 && shopDesc[scS[MENU_SHOP] * 4 + menuFocus]) {
		...
		case ITEM_QUEST:
		default:
			//상점에서 여러개 살수 있는 아이템을 사거나 파는 컨펌창인 경우
...
void SetActiveSkillString(int index, char* str, unsigned short width, unsigned short height)
{
	...
	for (i = 0, strPtr = &str[0]; i < 2; i++, skillValue++, strPtr = &tempStr2[0]) {
		...
		if (*skPtr2 == PASSIVE) {
			...
			case SKILL_ROBIN14:	//받는 데미지의 |i%d%%|b에 해당하는 MP를 소모하여 피해를 |r%d%%|b만큼 감소시킨다. %s
			case SKILL_ROBIN15:	//적에게 주는 데미지를 |r%d%%|b만큼 증가시킵니다. %s//회피 성공시 최대 HP의 |r%d%%|b만큼 HP를 회복한다. %s
			case SKILL_ROBIN16:	//최대 HP의 |i%d%%|b만큼 HP를 회복합니다. %s
			case SKILL_ROBIN17:	//피해를 입을 시 받은 데미지의 |r%d%%|b를 적에게 반사한다. %s
			case SKILL_DIANA14:	//방어력의 |r%d%%|b를 희생하면서 공격력을 |r%d%%|b만큼 증가시킨다. %s
			case SKILL_DIANA15:	//적에게 입힌 데미지의 |i%d%%|b만큼 MP를 회복한다. %s
			case SKILL_DIANA16:	//크리티컬 성공률이 |r%d%%|b만큼 증가한다. %s
			case SKILL_MAXX14:	//적에게 입힌 데미지의 |r%d%%|b만큼 HP를 회복한다. %s
			...
			case SKILL_DIANA17:	//적의 방어도를 |r%d%%|b만큼 무시하여 추가피해를 입힌다. %s
				...
				break;
			case SKILL_MAXX16:	//회피 성공시 |i%d|b만큼 MP를 회복한다. %s
...
void SetActiveSkillBodyString(int index)
{
	...
	for (i = 0, strPtr = &skillBodyStr[0]; i < 2; i++, skillValue++, strPtr = &tempStr2[0]) {
		...
		if (*skPtr2 == PASSIVE) {
			...
			switch (index) {
			default:
				//case SKILL_ROBIN6:		//아래에서부터 검을 위로 베어올리는 기술. %s
				//case SKILL_ROBIN7:		//날카로운 연속 찌르기로 적을 공격한다. %s
				//case SKILL_ROBIN8:		//가로로 검을 휘둘러 전방의 여러 적을 동시에 공격한다. %s
				//case SKILL_ROBIN9:		//몸통박치기로 적을 뒤로 날려버린다. %s
				//case SKILL_ROBIN10:	//칼등으로 내리쳐 적을 기절시킨다. %s
				//case SKILL_ROBIN11:	//검을 회전시키며 찌르는 강력한 찌르기. %s
				//case SKILL_ROBIN12:	//강력한 연속공격을 가하는 초필살기. %s
				//case SKILL_DIANA6:		//3방향으로 동시에 총을 발사한다. %s
				//case SKILL_DIANA7:		//지형에 따라 반사되는 레이저를 발사한다. %s
				//case SKILL_DIANA8:		//강력한 화염을 방사하여 적을 연타한다. %s
				//case SKILL_DIANA9:		//벽이나 적에 명중하면 폭발하는 로켓탄을 발사한다. %s
