...
bool DrawGloveInBox(int type, int detail, int x, int y, int w, int h, int alpha)
{
	int img = -1;
	const int d = Max(0, Min(detail, 7));
	if (type == ITEM_GUNTLET) {
		img = ITEM_GLOVE_ROBIN0_IMG + d;
	}
	else if (type == ITEM_ARMLET) {
		img = ITEM_GLOVE_DIANA0_IMG + d;
	}
	else if (type == ITEM_GLOVE) {
		img = ITEM_GLOVE_MAXX0_IMG + d;
	}
	else {
		return false;
	}
...
void DrawDiorama(int x, int y, int type, float zoom)
{
	...
	if (bossRaidMode && Abs(bossWorldScale - 1.0f) > 0.001f) {
	...
	else if (drawHandle == MD_PVP || gPvpCosmicTransition) {
		...
		const float targetZoom = gPvpCosmicTransition
			? gPvpCosmicBaseZoom : DIORAMAZOOM_BATTLE + dioramaZoomGap;
